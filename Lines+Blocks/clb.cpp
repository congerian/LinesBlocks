...
static void moveBall(int x1, int y1, int x2, int y2, int color) {
	...
	while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() < ts1 + moveTime) {
		...
		drawTexture(0, 0, width, height, text_clb, renderer);
		...
		drawNumber(692, 474, 16, 9, rec_clb);
		drawBalls();
		//drawOutline();
...
void clb() {
	...
	while (1) {
		...
		destroyBlocks();
		if (points > rec_clb) { rec_clb = points; setRecords(rec_cl, rec_clb, rec_sp, rec_spb); }
